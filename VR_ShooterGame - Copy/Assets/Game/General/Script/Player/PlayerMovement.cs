...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update() 
    {
        if (_realtimeView.isOwnedLocallyInHierarchy)
        {
            //playerSyncData.ChangedPlayerMoveInput(playerMovementInput.MoveInput);
            SetMoveDirection(playerMovementInput.MoveInput);
            ResetGravity();
            Move();
            SetMoveAnimation(playerMovementInput.MoveInput,moveSpeedMultiplier);
            Jump();
            SetGravity();
        }
    }
    ...
    public void SetMoveAnimation(Vector2 moveInput, float multiplier)
    {
        if (_realtimeView.isOwnedRemotelyInHierarchy)
        {
        animator.SetFloat("HorizontalMove", moveInput.x * multiplier, 0.05f, Time.deltaTime);
        animator.SetFloat("VerticalMove", moveInput.y * multiplier, 0.05f, Time.deltaTime);
        }
    }
